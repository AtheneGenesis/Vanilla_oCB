...
function ToFu:OnUpdate(timeSinceLast)
	...
	if not self.FlyStopped and not UnitOnTaxi("player") and (self.timeFlown > 5) then
		self.FlyStopped = 1
	elseif self.FlyStopped and not UnitOnTaxi("player") and (self.timeFlown > 5) then
		...
		self:SaveFlightData(self.start, self.destination, cost, time - 2, taken + 1)
